...
public class UserDao {
    ...
    public List<User> findWithFilter(String dateFilter) {
        StringBuilder sql = new StringBuilder("SELECT * FROM users WHERE 1 = 1");
        if ("today".equals(dateFilter)) {
            sql.append(" AND created_at >= CURDATE() AND created_at < CURDATE() + INTERVAL 1 DAY");
        }
    ...
    public List<User> getUsersByRole(String role) {
        String sql = "SELECT * FROM users WHERE role = :role AND is_active = 1";
        return jdbi.withHandle(handle -> handle.createQuery(sql)
                .bind("role", role)
                .mapToBean(User.class)
                .list());
