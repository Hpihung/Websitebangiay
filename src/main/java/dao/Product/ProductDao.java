...
public class ProductDao {
    ...
    public int insert(Product product) {
        String sql = """
                    INSERT INTO product(name, description, price, brand_id,
                                        added_at, is_discontinue, is_available, status)
                    VALUES(:name, :description, :price, :brandId,
                           NOW(), 0, :available, :status)
                """;
    ...
    public List<Product> filterProducts(String keyword, List<Integer> brandIds,
        ...
        String orderBy = switch (sortBy != null ? sortBy : "default") {
            ...
            case "bestseller" -> " ORDER BY (SELECT COALESCE(SUM(od.quantity), 0) FROM order_detail od JOIN orders o ON od.order_id = o.id WHERE od.product_id = p.id AND o.order_status != 'ORDER_CANCELLED') DESC, p.added_at DESC ";
    ...
    public List<Product> findAllPaged(int limit, int offset) {
        String sql = """
                SELECT *
                FROM product
                WHERE is_discontinue = 0
    ...
    public int countAll() {
        String sql = "SELECT COUNT(*) FROM product WHERE is_discontinue = 0";
        return jdbi.withHandle(handle -> handle.createQuery(sql)
                .mapTo(int.class)
                .one());
    }
