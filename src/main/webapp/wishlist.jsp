...
<head>
    ...
    <title>Wishlist - H&M SPORT SHOES</title>
                    ...
                    <div class="product-details">
                        <c:choose>
                            <c:when test="${not empty p.mainImageUrl && p.mainImageUrl.startsWith('http')}">
                                <img src="${p.mainImageUrl}" alt="${p.name}" class="product-image" />
                            </c:when>
                            <c:when test="${not empty p.mainImageUrl}">
                                <img src="${pageContext.request.contextPath}${p.mainImageUrl}" alt="${p.name}" class="product-image" />
                            </c:when>
                            <c:otherwise>
                                <img src="https://placehold.co/300x300?text=No+Image" alt="${p.name}" class="product-image" />
                            </c:otherwise>
                        </c:choose>
